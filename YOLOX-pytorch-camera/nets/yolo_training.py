...
from nets.varifocalloss import VarifocalLoss
...
class YOLOLoss(nn.Module):    
    ...
    #---------------varifocalloss----------------------
        self.varifocal_loss = VarifocalLoss(reduction='none')
    #---------------focalloss-------------------------
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        # ——————————————改进focal_loss
        # loss_obj    = (self.focal_loss(obj_preds.sigmoid().view(-1, 1), obj_targets)).sum()
        # ———————————————改进variFocal_loss
        #loss_obj    = (self.varifocal_loss(obj_preds.sigmoid().view(-1, 1), obj_targets)).sum()
        # ————————————————原始bceloss
